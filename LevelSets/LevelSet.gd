...
func sandbox(): 
	...
	var raw_enemies = {0:{Vector2(2, 2): make(Drummer, 3, [unstable]), 
	Vector2(3, 3): make(Ranged, 3), Vector2(3, 4): make(Ranged, 3), Vector2(4, 6): make(Ranged, 3)}}

	var extras1 = {"free_deploy":false, "flags":flags, "score_guide":score_guide}
	
	var enemies = EnemyWrappers.FiniteCuratedWrapper.new(raw_enemies)
	var heroes = {1:Berserker, 2: Archer, 5: Corsair, 4:Stormdancer} 
	return LevelTypes.Timed.new(33333, "Sandbox", heroes, enemies, 4, null, extras1) 
#	
func background():
	var pieces = load_level("reinforcement_test.level")
