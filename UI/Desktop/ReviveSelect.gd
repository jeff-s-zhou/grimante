...
func initialize(dead_piece):
	#we only show this screen if we have pieces to select lol
	self.pieces.append(dead_piece)
	dead_piece.set_pos(Vector2(-999, -999)) #get it out of the way
	
	var mock_piece = FinalHeroMockPiecePrototype.instance()
	mock_piece.initialize(dead_piece)
...
func select(piece):
	...
	self.grid.add_piece(coords, self.currently_selected, true)
