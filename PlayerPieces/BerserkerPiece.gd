...
func _ready():
	...
	get_node("Tween").connect("tween_complete", self, "animation_end")
...
func jump_to(new_coords, speed=4):
	...
	get_node("Tween").interpolate_property(self, "transform/pos", get_pos(), new_position, time, Tween.TRANS_LINEAR, Tween.EASE_IN)
	get_node("Tween").interpolate_callback(self, time, "animate_smash")
