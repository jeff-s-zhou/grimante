...
func receive_shove(shover, distance, damage):
	...
	#will land on something
		...
		var action = shover.get_new_action()
		if get_parent().pieces[new_coords].side == "ENEMY":
			action.add_call("attacked", [damage, shover], [new_coords])
		else:
			action.add_call("attacked", [shover], [new_coords])
		action.execute()
